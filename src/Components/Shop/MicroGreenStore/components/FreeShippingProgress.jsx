...
const FreeShippingProgress = ({ amountLeft }) => {
  const totalAmount = 499; // Threshold for free shipping
  const progress = Math.min(((totalAmount - amountLeft) / totalAmount) * 100, 100);
  
  const eligible = amountLeft <= 0;
    ...
    <div className="p-4 max-w-md mx-auto">
      <p className={`text-lg ${eligible && `text-green-600`} font-semibold text-center mb-2`}>
        {eligible 
          ...
          : `Add products worth ₹${amountLeft} to avail free shipping`}
      </p>
